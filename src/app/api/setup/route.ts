...
export async function POST() {
  try {
    ...
    if (count > 0) {
      return NextResponse.json({ message: 'Tabelas criadas. Banco já possui dados — seed ignorado.' });
    }

    await seed();

    return NextResponse.json({ message: 'Setup completo! Tabelas criadas e dados inseridos.' });
  } catch (error) {
